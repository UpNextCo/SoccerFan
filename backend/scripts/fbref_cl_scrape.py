"""
Direct FBref Champions League player-stats scraper (pre-2010).

soccerdata can't parse cup competitions, so this fetches the FBref CL "Standard
Stats" page itself via SeleniumBase's undetected-Chrome driver (Cloudflare bypass)
and parses the player table with pandas. Output feeds job:import-fbref.

Usage:
  source .venv/bin/activate
  python3 scripts/fbref_cl_scrape.py            # 1991-2009 by default
  python3 scripts/fbref_cl_scrape.py 2000 2009

Then:
  npx tsx src/jobs/import-fbref.ts fbref_cl_backfill.json

If every season returns 0 rows (Cloudflare), re-run with a visible browser:
  HEADLESS=0 python3 scripts/fbref_cl_scrape.py
"""
import json
import math
import os
...
# (FBref comp id, our leagueId matching api-football, league name)
COMPS = [
    (8, 2, "UEFA Champions League"),
    # (19, 3, "UEFA Europa League"),  # uncomment to also try Europa/UEFA Cup
]

OUT_PATH = "fbref_cl_backfill.json"
REQUEST_DELAY_S = 4
...
def main():
    ...
    rows = []
    try:
        for comp_id, lid, lname in COMPS:
            for year in range(start, end + 1):
                season = f"{year}-{year + 1}"
                url = f"https://fbref.com/en/comps/{comp_id}/{season}/stats/{season}-Champions-League-Stats"
                try:
